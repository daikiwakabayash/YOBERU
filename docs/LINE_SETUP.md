...
> ⚠️ **本番 URL について**:
> このドキュメントでは Vercel 本番 URL を `https://yoberu-eight.vercel.app` として記載しています。
> もし独自ドメイン（例: `app.yurumu8.net`）に切り替えた場合は、ドキュメント内の URL を全て読み替えてください。
> なお `yoberu.vercel.app` は別人がすでに使用している無関係なサービスなので、絶対に使用しないでください。
...
| Key | Value |
|---|---|
| `NEXT_PUBLIC_APP_URL` | `https://yoberu-eight.vercel.app` |

> 💡 もし Vercel の Domains 設定で `yoberu-eight.vercel.app` 以外が割り当てられている場合（例: `yoberu-daikiwakabayashis-projects.vercel.app`）はそちらを使ってください。確認: Project → Settings → **Domains** に表示されている URL。
...
```
https://yoberu-eight.vercel.app/api/line/webhook
```
...
| **Size** | **Full** （全画面）|
| **Endpoint URL** | `https://yoberu-eight.vercel.app/line/liff` |
...
ブラウザで `https://yoberu-eight.vercel.app/login` にアクセス → 管理者アカウントでログイン。
...
npx ts-node scripts/setup-line-rich-menu.ts \
  ...
  --base=https://yoberu-eight.vercel.app
...
### 8-1. ログイン確認
- [ ] `https://yoberu-eight.vercel.app/login` でログインできる
...
- [ ] Webhook URL 登録済み（`https://yoberu-eight.vercel.app/api/line/webhook`）
...
https://yoberu-eight.vercel.app/line/liff

# YOBERU ログイン URL
https://yoberu-eight.vercel.app/login

# YOBERU 店舗設定
https://yoberu-eight.vercel.app/store

# YOBERU LINE チャット
https://yoberu-eight.vercel.app/line-chat
