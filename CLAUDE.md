...
- **attribution は `getMarketingData` (媒体別内訳) と完全一致させてある**ので
  クリエイティブ分析の合計 = 媒体別内訳の合計 になる:
  - 新規 (= 予約数) = 顧客の人生最古予約 (status 不問) が当期間にある人数 (1 顧客 = 1 新規)
  - 実来院 = その最古予約が status=2 / キャンセル = status ∈ {3,4,99}
  - 売上 = 顧客の人生 1〜3 回目 status=2 予約の sales (当期間分)
- 広告費は creative 単位 (`booking_link_id NOT NULL`) と媒体単位
  (`booking_link_id NULL`) の両方を対象。前者は該当クリエイティブ行、
  後者は「未割当」行に計上する (合計は媒体別の広告費と一致)

#### 未割当 (強制リンクなし) バケット
`appointments.booking_link_id` を持たない予約 (= 手動登録 / 媒体直来店 /
migration 00052 以前の予約) は特定クリエイティブに紐付けられないため、
**(店舗 × 媒体) 単位の「未割当」バケット**にまとめて計上する。これにより
全体合計が媒体別内訳と一致し、「どれだけがクリエイティブに attribution
できていないか」が可視化される。過去レコードを機械的に特定クリエイティブへ
割り当てると (媒体→クリエイティブは 1:多なので) 誤 attribution になるため、
あえて未割当として束ねる方針。症状 / オファー価格で絞り込み中は未割当行は
非表示。

### UI (クリエイティブ分析タブ)
ピボット表で:
- 行 = 店舗 × 媒体 × 症状 × オファー価格 (1 行が 1 クリエイティブ群)
- 列 = 予約数 / 実来院 / 入会数 / 入会率 / キャンセル率 / 広告費 / CPA / 売上 / ROAS
- 「未割当」行は症状列にグレーの `未割当` バッジを出し、常に表の最下部に表示
